    ...
    try {
            ...
            playerEntries.forEach(([name, data]) => {
                ...
                const movement =
                    data.rankMovement > 0
                        ? `↑${data.rankMovement}`
                        : data.rankMovement < 0
                          ? `↓${Math.abs(data.rankMovement)}`
                          : '─';

                console.log(
                    `${rank.toString().padStart(2)}: ${name.padEnd(20)} | ${points.toString().padStart(6)} pts | ${appearances} games | ELO: ${eloRating.padStart(4)} | ${movement}`
                );
            });
        }

        // Check for players with ELO data to verify fix worked
        const playersWithElo = Object.entries(players).filter(
            ([, data]) => data.elo && typeof data.elo.rating === 'number'
        ).length;
        ...
        if (playersWithElo > 0) {
            ...
            eloExamples.forEach(([name, data]) => {
                console.log(
                    `  ${name}: ${data.elo.rating.toFixed(0)} (${data.elo.gamesPlayed || 0} games played)`
                );
            });
        } else {
