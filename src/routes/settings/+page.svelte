...
<form class="flex gap-2" onsubmit={async (event) => await saveSettings(event)}>
	<Input
...
<Toggle bind:checked={storedSettings.canRegenerateTeams} onchange={saveSettings}
	>Allow team regeneration</Toggle
>
<Toggle bind:checked={storedSettings.canResetSchedule} onchange={saveSettings}
	>Allow schedule reset</Toggle
>
