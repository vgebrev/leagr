...
{:else}
    <Table
        ...
        <TableHead class="dark:text-gray-300">
            <TableHeadCell class="w-8 px-1 py-1.5"
            ...
            <TableHeadCell class="w-px px-1 py-1.5 text-center sm:w-12">Exp</TableHeadCell>
            <TableHeadCell class="w-px px-1 py-1.5 text-center sm:w-12">Price</TableHeadCell>
            <TableHeadCell class="w-8 px-1 py-1.5"><span class="sr-only">Pick</span></TableHeadCell>
        </TableHead>
        <TableBody>
            {#each market as entry (entry.playerName)}
                ...
                <TableBodyRow
                    ...
                    <TableBodyCell class="w-full max-w-0 px-0 py-1">
                        <span
                    ...
                    <TableBodyCell class="px-1 py-1 text-center text-gray-500 dark:text-gray-400">
                        {entry.expectedPoints === null || entry.expectedPoints === undefined
                            ? '—'
                            : Math.round(entry.expectedPoints * 10) / 10}
                    </TableBodyCell>
                    <TableBodyCell class="px-1 py-1 text-center font-bold">
                        {entry.price}
                    </TableBodyCell>
                    <TableBodyCell class="px-1 py-1 text-center">
                        {#if isPicked}
