...
<div class="flex flex-col gap-2">
  ...
  <div class="flex w-full flex-col gap-2">
    {#each teamConfig as config, i (i)}
      <div class="rounded-md border p-2">
        <Radio bind:group={selectedTeamConfig} value={config}
  ...
  {#if confirmRegenerate}
    <Alert class="flex items-center border"
      ...
      regenerate them?
      <Button size="sm" onclick={async () => await generateTeams(true)}>Yes</Button></Alert
    >
  {/if}
  <div class="grid grid-cols-2 gap-2">
    {#each Object.entries(teams) as [teamName, team], i (i)}
      <TeamTable {team} {teamName} teamIndex={i} onremove={removePlayer} {players} />
    {/each}
    {#if waitingList?.length > 0}
      <TeamTable team={waitingList} color="gray" teamName="Waiting List" {players} />
    {/if}
  </div>
</div>
