...
{#if confirmRegenerate}
	<Alert class="flex items-center border"
		><ExclamationCircleSolid />Games have already been schedule. Are you sure you want to reset the
		schedule?
		<Button size="sm" onclick={async () => await scheduleGames(true)}>Yes</Button></Alert
	>
{/if}
		...
		<Listgroup class="w-full shadow">
			...
			{#each round.filter((m) => !m.bye) as matchup, j (j)}
				<ListgroupItem class="flex justify-between gap-2 p-2">
					<TeamBadge className="w-2/5" teamName={matchup.home} {teams} />
					<Input
						type="number"
						size="sm"
					...
					<Input
						...
						onchange={saveScore}
					/>
					<TeamBadge className="w-2/5" teamName={matchup.away} {teams} />
				</ListgroupItem>
			{/each}
		</Listgroup>
	{/each}
