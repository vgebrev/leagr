				...
				<TableBodyRow>
					...
					<TableBodyCell class="px-1 py-1.5 text-center"
						><div class="flex justify-between">
							<TeamBadge className="w-full" teamName={team.team} {teams} />
						</div>
					</TableBodyCell>
					<TableBodyCell class="px-1 py-1.5 text-center">
						{team.played}
...
{:else}
	<div class="py-2">
		Schedule some <Button color="alternative" href="/games?date={data.date}" size="xs"
			><CalendarMonthSolid class="me-2 h-4 w-4"></CalendarMonthSolid>Games</Button
		> and enter their scores to see the standings.
	</div>
{/if}
