<script>
  ...
  let { team, teamIndex, color, teamName, onremove, players } = $props();
          ...
          <td class="m-0 p-2"
            ><div class="flex">
              ...
              {:else if player && !players.includes(player)}
                <span class="flex gap-2 line-through"><ExclamationCircleSolid />{player}</span>
              {:else}
                <span class="flex gap-2 italic"><ExclamationCircleSolid /> Empty</span>
              {/if}
              {#if onremove}
                <Button
                  size="sm"
                  ...
                  ><svg class="h-4 w-4" width="20" height="20" viewBox="0 0 20 20"
                    ><path
